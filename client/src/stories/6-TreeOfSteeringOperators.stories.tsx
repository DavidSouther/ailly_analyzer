...
const OPERATORS: Record<OperatorKey, OperatorMeta> = {
  prompting: {
    ...
    referentCheck: "None within the generation, human or agent operator when used in a harness",
    flagged: true,
  },
  reasoning: {
  ...
  toolCalls: [
    {
      id: "root-1",
      tool: "Agent",
      summary: "Broke the request into a locate -> migrate -> runbook -> review chain",
    },
  ],
  children: [LOCATE],
};

function summarizeToolCalls(calls: ToolCallEntry[]): { total: number; mostly: ToolName } | null {
  if (calls.length === 0) {
    return null;
  }
  ...
  let mostly: ToolName = calls[0]?.tool ?? "Read";
  let max = 0;
...
function ToolCallSummaryRow({ toolCalls }: { toolCalls: ToolCallEntry[] }) {
  ...
  if (!summary) {
    return (
      <MutedText className="text-xs">This step hasn&apos;t made any tool calls yet.</MutedText>
    );
  }
        ...
        <PanelAccordion className="ml-4 rounded-sm border">
          {toolCalls.map((call) => {
            ...
            return (
              ...
              >
                <p className="p-2 text-xs text-foreground-muted">{call.summary}</p>
              </PanelAccordionItem>
            );
          })}
        </PanelAccordion>
  ...
  return (
    <div className="flex flex-col gap-1 rounded-sm border bg-background p-2">
      <div className="flex items-center gap-1.5">
        ...
        {meta.flagged ? (
          <Badge color={BadgeColor.BERRY} outlined={true} textSize="sm" icon={TriangleAlertIcon}>
      ...
      <dl className="grid grid-cols-[5.5rem_1fr] gap-x-2 gap-y-1 text-xs">
        ...
        <dt className="tracking-wide text-foreground-muted uppercase">Signal</dt>
        <dd className="m-0 text-foreground">{meta.signal}</dd>
        <dt className="tracking-wide text-foreground-muted uppercase">Referent check</dt>
        <dd
          className={cn("m-0", meta.flagged ? "font-medium text-status-error" : "text-foreground")}
        >
          {meta.referentCheck}
        </dd>
      </dl>
...
const DEPTH_PADDING = ["pl-0", "pl-5", "pl-10", "pl-14", "pl-[4.5rem]", "pl-20"];
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        {isBranch ? <GitForkIcon className="h-3 w-3 shrink-0 text-foreground-muted" /> : null}
        <StrongText
          className={cn(
            "flex-1 text-sm",
        ...
        <MutedText className="shrink-0 text-xs">{STATUS_LABEL[node.status]}</MutedText>
      </button>
      {expanded ? (
        <div
          className={cn(
        ...
        <div>
          {node.children.map((child) => (
            <TreeNode
              ...
              node={child}
              depth={depth + 1}
              isBranch={(node.children?.length ?? 0) > 1}
            />
          ))}
        </div>
      ) : null}
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-3 bg-background p-6">
      <BodyText className="text-xs text-foreground-muted">
        The Orchestrator TODO List, regrown as a tree. Locate -&gt; Migrate -&gt; Runbook -&gt;
        Review form a serial chain; Migrate itself fans out into three candidate implementations
        (Multi-sample branching) that converge into one selected approach before the chain
        continues. Every node is tagged with a steering operator from the paper&apos;s Table 1 --
        expand a node for its Move / Signal / Referent-check triplet and a compact tool-call summary
        for that step. Operators whose referent check is{" "}
        <StrongText className="text-xs">None</StrongText> (Prompting, Additional reasoning tokens)
        or reduces to <StrongText className="text-xs">depends on the selector</StrongText>{" "}
        (Multi-sample branching) carry a warning badge -- that&apos;s the paper&apos;s specific
        diagnostic point, not an omission.
      </BodyText>
