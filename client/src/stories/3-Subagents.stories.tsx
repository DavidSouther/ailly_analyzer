...
const TRANSCRIPT: TranscriptItem[] = [
  ...
  },
  {
    ...
    status: "done",
    detail: "import { LegacySession } from '@nominal-io/auth';\n// 2 more call sites below",
  },
  {
    kind: "orchestrator",
    time: "10:41:52",
...
function ToolRow({ item }: { item: ToolItem }) {
  ...
  return (
    <div className="flex items-start gap-1.5">
      ...
      <span className="mt-1 shrink-0 text-[10px] text-foreground-muted/60">{item.time}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex max-w-xl flex-col gap-3 bg-background p-6">
      <div className="text-xs text-foreground-muted">
        Era 3 — subagents run concurrently, but their tool calls still render inline, one after
        another, in a single transcript. Try tracing which agent did what.
      </div>
