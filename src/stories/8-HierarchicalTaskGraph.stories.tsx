...
function computeRanks(): Map<string, number> {
  ...
  while (changed) {
    ...
    for (const task of TASKS) {
      ...
      const consumerRanks = (consumers.get(task.id) ?? []).map((id) =>
        rank.get(id),
      );
...
const TASK_TREE_BOXES: Box[] = TASKS.map((task) => {
  const spawnedBy = TASKS.find(
    (candidate) =>
      ...
      !(candidate.reused?.includes(task.id) ?? false),
  );
...
function boxesFrom(hulls: Box[], placement: Record<string, string | null>) {
  return [
    ...hulls,
    ...TASKS.map((task) => taskBox(task.id, placement[task.id] ?? null)),
  ];
}
...
function buildGraph(hierarchy: Hierarchy): HierarchyGraph {
  ...
  for (const box of hierarchy.boxes) {
    if (box.parent == null) {
      ...
      childrenOf.set(box.parent, [
        ...(childrenOf.get(box.parent) ?? []),
        box.id,
      ]);
    }
  }
  ...
  for (const box of hierarchy.boxes) {
    ...
    for (const ancestor of ancestorsOf.get(box.id) ?? []) {
      tasksUnder.set(ancestor, [
        ...(tasksUnder.get(ancestor) ?? []),
        box.taskId,
      ]);
    }
  }
  ...
  for (const box of hierarchy.boxes) {
    const ranks = (tasksUnder.get(box.id) ?? []).map(
      (taskId) => RANK.get(taskId) ?? MAX_RANK,
    );
  ...
  const order = (a: string, b: string) => {
    ...
    return byRank !== 0
      ? byRank
      : (boxById.get(a)?.label ?? "").localeCompare(
          boxById.get(b)?.label ?? "",
        );
  };
...
function defaultCutDepth(graph: HierarchyGraph): number {
  for (let depth = 0; depth <= graph.maxDepth; depth++) {
    const groups = new Set(
      TASKS.map((task) => ancestorAtDepth(graph, task.id, depth)),
    );
    if (groups.size >= 3) {
      return depth;
    }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center gap-1">
        <MutedText className="mr-1 text-xs tracking-wide uppercase">
          Hierarchy
        </MutedText>
    ...
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-border pt-2">
      <span className="flex items-center gap-1.5 text-xs text-foreground-muted">
        <span
          ...
          style={{ background: EDGE_COLOR, opacity: 0.7 }}
        />
        one segment; the number on it is how many edges it bundles
      </span>
      {note ? (
        <span className="text-xs text-foreground-muted">{note}</span>
      ) : null}
    </div>
  );
}
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isHull
          ? "border border-dashed border-border bg-background-hover/30 p-2"
          : "",
      )}
    >
      {box.taskId ? (
        ...
        >
          ...
          <StrongText className="text-xs text-foreground-muted">
            {box.label}
          </StrongText>
        </button>
      )}
      ...
      {isHull && !isOpen ? (
        <MutedText className="mt-1 self-start px-1 text-[10px]">
          ...
          {hiddenEdges > 0
            ? ` · ${hiddenEdges} internal edge${hiddenEdges === 1 ? "" : "s"}`
            : ""}
        </MutedText>
      ) : null}
...
function routeSegment(
  ...
  const nodeOf = (id: string) => {
    const isExpandedTaskHull =
      graph.boxById.get(id)?.taskId != null &&
      !(graph.hullIds.has(id) && collapsed.has(id));
  ...
  if (toIndexInUp > 0) {
    return routeToAncestor(
      upChainAll.slice(0, toIndexInUp),
      fromNode,
      toNode,
      rectOf,
    );
...
function routeToAncestor(
  ...
  return [
    borderPoint(innerNode, first, 2),
    ...ports,
    borderPoint(outerNode, last, 2),
  ];
}
      ...
      {chrome ? (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <MutedText className="text-xs">
            ...
            {segmentSet.segments.length === 1 ? "" : "s"} bundling{" "}
            {EDGES.length} edges
          </MutedText>
        </div>
      ) : null}
...
function topLevelCollapse(graph: HierarchyGraph): string[] {
  ...
  return [...graph.hullIds].filter(
    (id) => (graph.depthOf.get(id) ?? 0) >= floor,
  );
}
...
function BandsView({ hierarchyId }: { hierarchyId: string }) {
  const graph = GRAPHS.get(hierarchyId);
  const [cutDepth, setCutDepth] = useState(() =>
    graph ? defaultCutDepth(graph) : 0,
  );
  const containerRef = useRef<HTMLDivElement>(null);
  const { register, measure, rects } = useRectRegistry(containerRef);
  ...
  const drawn: DrawnSegment[] = useMemo(() => {
    ...
    for (const [key, edges] of bundles) {
      ...
      const anchors = edges
        ...
        .filter(
          (pair): pair is { from: Rect; to: Rect } =>
      ...
      const waypoint = {
        x:
          anchors.reduce(
            (sum, pair) => sum + (pair.from.x + pair.from.w + pair.to.x) / 2,
            0,
          ) / anchors.length,
        y:
          anchors.reduce(
            (sum, pair) =>
              sum +
              (pair.from.y + pair.from.h / 2 + pair.to.y + pair.to.h / 2) / 2,
            0,
          ) / anchors.length,
      };
      for (const pair of anchors) {
...
function OutlineView({ hierarchyId }: { hierarchyId: string }) {
  ...
  const rows = useMemo(
    () => (graph ? flattenOutline(graph, collapsed) : []),
    [graph, collapsed],
  );
  const segmentSet = useMemo(
    () => (graph ? computeSegments(graph, collapsed) : null),
  ...
  const drawn: DrawnSegment[] = useMemo(() => {
    ...
    for (const segment of segmentSet.segments) {
      ...
      const sharedDepth =
        shared == null ? -1 : (graph.depthOf.get(shared) ?? 0);
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <MutedText className="text-xs">
          Arc swing = depth of the hull the two ends share.
        </MutedText>
      </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="rounded-sm border border-border bg-background-hover/40 p-2">
        {selected == null ? (
          <MutedText className="text-xs">
            Row feeds column. A solid cell is a segment between two hulls; a
            dashed cell on the diagonal is the traffic a hull keeps to itself.
            Pick a cell to see the edges it bundles.
          </MutedText>
        ) : (
          <div className="flex flex-col gap-1">
            ...
            {selectedEdges.map((edge) => (
              ...
              >
                <span className="truncate">
                  {TASK_BY_ID.get(edge.from)?.label}
                </span>
                <ChevronRightIcon className="h-3 w-3 shrink-0" />
                <span className="truncate">
                  {TASK_BY_ID.get(edge.to)?.label}
                </span>
              </div>
            ))}
          </div>
...
export function Containment() {
  ...
  return (
    <Frame
      ...
      intro={
        <>
          The same 17 tasks and 20 edges as sketch 7, drawn inside a containment
          tree. Nothing about the topology changed; what changed is that an edge
          crossing a hull boundary now leaves through a port shared with every
          other edge making the same crossing. Collapse a hull and its edges do
          not disappear &mdash; they merge into the parent&apos;s segments, and
          the count of internal edges it swallowed is printed on the hull.{" "}
          <StrongText className="text-xs">Try Task Tree: </StrongText>because
          that hierarchy is the spawn tree, every line just runs from a task up
          to the header of a hull that already contains it &mdash; parentage
          restated as geometry. The only lines crossing sideways between hulls
          are the four reuse edges, which is the wiring a reader could not have
...
export function CollapsedFirst() {
  ...
  return (
    <Frame
      ...
      intro={
        <>
          Every hull starts collapsed, so the first thing on screen is the shape
          of the whole run rather than 17 cards. Under the{" "}
          <StrongText className="text-xs">Session </StrongText>hierarchy each
          hull is one long-running agent, so each segment is something one agent
          had to tell another and each internal count is work an agent finished
          without leaving its own session. Expanding a hull grows it in place;
          the other hulls do not move, and the segments it was part of split
          into the finer segments underneath.
        </>
      }
    >
      <ContainmentView
...
export function SegmentMatrix() {
  ...
  return (
    <Frame
      ...
      intro={
        <>
          What is left of the picture when every hull is closed: rows feed
          columns, and the cell is the segment weight. No node positions, no
          routing, no crossing lines -- and it stays readable at a hull count
          where the drawn forms will not. Under{" "}
          <StrongText className="text-xs">Session </StrongText>this is a handoff
          ledger; under <StrongText className="text-xs">Path </StrongText>it is
          a coupling matrix over the repository.
        </>
      }
    >
...
export function HierarchyVariants() {
  return (
    <div className="mx-auto flex max-w-[1500px] flex-col gap-6 bg-background p-6">
      <BodyText className="text-xs text-foreground-muted">
        One topology, four containment trees. A DAG does not come with a
        hierarchy, so each of these is a choice, and each choice needs a
        tie-break for tasks with more than one consumer. Read the segments as
        the cost of the choice: they are the edges the hierarchy failed to
        localize.
      </BodyText>
      {HIERARCHIES.map((hierarchy) => {
...
export function BandsRejected() {
  ...
  return (
    <Frame
      ...
      intro={
        <>
          Lanes are the hierarchy cut at a fixed depth; columns are DAG rank, so
          the flow still reads left to right into the goal. There is no nesting
          to hang a port on, so each pair of lanes gets one bundle waypoint and
          every edge between them is pinched through it. Raise the cut depth to
          split lanes into their sub-hulls.
        </>
      }
    >
      <RejectedBanner>
        The picture is mostly empty and gets emptier as hulls are added: its
        area is lanes &times; ranks while the content stays at 17 cards, because
        a lane only occupies the ranks it happens to own. Worse, there is no
        hull boundary to cross, so the bundle point is invented rather than
        derived &mdash; which means this form cannot draw nesting at all. The
        cut-depth control is not a feature here, it is the admission: the tree
        is flattened to one level and everything below it is discarded.
      </RejectedBanner>
...
export function OutlineRejected() {
  ...
  return (
    <Frame
      ...
      intro={
        <>
          The narrow-panel form. Indentation carries the hierarchy and the arcs
          carry the topology, with each arc bowing out in proportion to how far
          up the tree its two ends have to reach to meet. Under{" "}
          <StrongText className="text-xs">Path </StrongText>the hierarchy is the
          repository layout, which has nothing to do with the DAG, so the arcs
          are long and cross-cutting.
        </>
      }
    >
      <RejectedBanner>
        Every segment has to fit in one vertical gutter, so at 20 edges the arcs
        already overlap into a single smear and there is no way to follow one to
        its other end. The arc swing encodes the depth of the shared hull, which
        is a second thing to decode on top of that. And row order is tree order,
        not rank order, so the direction of the work &mdash; the thing sketch 7
        made legible &mdash; is no longer visible anywhere on the page.
      </RejectedBanner>
