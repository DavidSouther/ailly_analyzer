...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <MutedText className="text-xs font-medium tracking-widest uppercase">
      {children}
    </MutedText>
  );
}
  ...
  if (spawns === 0) {
    return (
      <MutedText className="text-xs">
        No subagents were spawned this session.
      </MutedText>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 divide-x divide-y divide-border rounded-md border sm:grid-cols-4">
        ...
        <StatTile
          label="Files touched"
          value={stats.totalDistinctFiles.toLocaleString()}
        />
        <StatTile label="Duration" value={stats.durationLabel} />
        <StatTile
          label="Subagent spawns"
          value={stats.subagentSpawns.toLocaleString()}
        />
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <FilesTouchedList
          files={stats.files}
          totalDistinctFiles={stats.totalDistinctFiles}
        />
      </div>
      <div className="flex flex-col gap-1.5">
        <SectionHeading>Subagents</SectionHeading>
        <SubagentBreakdown
          spawns={stats.subagentSpawns}
          types={stats.subagentTypes}
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-4 bg-background p-6">
      <BodyText className="text-xs text-foreground-muted">
        Post-session Tool Call Summary -- after the orchestrator and its
        subagents finish, this rolls every tool call from the whole session up
        into one auditable view: which tools ran, how often, and which files
        they touched. This is the after-the-fact companion to the Orchestrator
        TODO List&apos;s in-progress view, not a replacement for it.
      </BodyText>
...
export function LargeSession() {
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-4 bg-background p-6">
      <BodyText className="text-xs text-foreground-muted">
        The same summary view under a long, tool-heavy session — 268 calls
        across 47 files and 9 subagent spawns — to check that the rollup still
        reads cleanly at scale instead of only working for toy examples.
      </BodyText>
  ...
  },
  {
    ...
    sample: [
      ...
      },
      {
        ...
        targetKind: "file",
        detail:
          "Read the existing runbook structure before drafting the new section.",
      },
      {
        id: "read-4",
        agent: "Orchestrator",
        ...
        targetKind: "file",
        detail:
          "Read to confirm no other call sites were missed by the search.",
      },
    ],
  },
  {
...
const SOURCES_TOTAL = SOURCE_GROUPS.reduce(
    ...
    sample: [
      {
        ...
        targetKind: "file",
        detail:
          "Renamed LegacySession to SessionStore and added a static fromToken() constructor.",
      },
      {
        id: "edit-2",
        agent: "Subagent 1",
        ...
        targetKind: "file",
        detail:
          "Swapped the LegacySession import for SessionStore.fromToken().",
      },
      {
        id: "edit-3",
        agent: "Subagent 2",
    ...
    sample: [
      {
        ...
        targetKind: "file",
        detail:
          "New test file covering SessionStore.fromToken() and expiry handling.",
      },
      {
        id: "write-2",
        agent: "Subagent 1",
      ...
      },
      {
        ...
        targetKind: "file",
        detail:
          "One-off script to re-encode sessions stored under the legacy key format.",
      },
    ],
  },
  {
    ...
    sample: [
      ...
      },
      {
        ...
        targetKind: "subagent",
        detail:
          'Task: "Migrate packages/auth/src/session.ts to SessionStore and add tests."',
      },
      {
        id: "agent-3",
        agent: "Orchestrator",
        ...
        targetKind: "subagent",
        detail:
          'Task: "Review the session-store migration diff for backward-compatibility risk."',
      },
    ],
  },
  {
    ...
    sample: [
      {
        ...
        agent: "Orchestrator",
        target:
          "Keep LegacySession as a deprecated alias, or remove it outright?",
        targetKind: "other",
...
function CallAccordionItem({ call }: { call: CallDetail }) {
  ...
  return (
    <PanelAccordionItem
      ...
      actions={
        <Badge
          color={TARGET_KIND_COLOR[call.targetKind]}
          textSize="sm"
          icon={Icon}
        >
        ...
        <div className="border-t border-border bg-background-hover/40 px-3 py-2 pl-8">
          ...
          {hidden > 0 ? (
            <MutedText className="mt-1.5 text-xs">
              +{hidden} more {group.tool} call{hidden === 1 ? "" : "s"} not
              shown
            </MutedText>
          ) : null}
        </div>
...
export function ByTool() {
  return (
    <div className="mx-auto flex max-w-xl flex-col gap-3 bg-background p-6">
      <BodyText className="text-xs text-foreground-muted">
        The same session, regrouped by tool instead of by task — the drill-down
        the design doc calls out specifically: which tools were called with what
        parameters. Bash, Read, and WebFetch are rolled into a single{" "}
        <StrongText className="text-xs">Sources</StrongText> group — every way a
        fact enters context from outside the conversation. Expand it for the
        breakdown by type, then expand a type (e.g. File reads) to see exactly
