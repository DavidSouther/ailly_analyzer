...
function computeRanks(nodes: GraphNode[], goalId: string): Map<string, number> {
  ...
  while (changed) {
    ...
    for (const node of nodes) {
      ...
      if (
        consumers.length === 0 ||
        consumerRanks.some((r) => r === undefined)
      ) {
        continue;
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <StrongText
          className={cn("flex-1 truncate", isTool ? "text-xs" : "text-sm")}
        >
          {node.label}
        </StrongText>
        {expanded ? (
        ...
        <defs>
          ...
          >
            <path
              ...
              style={{ fill: TONE_COLOR.highlight }}
            />
          </marker>
          <marker
          ...
          >
            <path
              d="M0,0 L10,5 L0,10 Z"
              style={{ fill: TONE_COLOR.rejected }}
            />
          </marker>
        </defs>
        {edges.map((edge) => (
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-4 bg-background p-6">
      <BodyText className="text-xs text-foreground-muted">
        A DAG with one sink: the goal, at the top. Every other node's edge
        points toward whatever it supports. Watch{" "}
        <StrongText className="text-xs">Update the runbook</StrongText> -- while
        running, it needed the current call sites of LegacySession, found{" "}
        <StrongText className="text-xs">Locate call sites</StrongText> already
        answered that, and wired a new edge to reuse it instead of asking again.
        That node now feeds two consumers. Tool calls (the smaller, muted cards)
        are ordinary nodes in the same graph, not a separate category.
      </BodyText>
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-4 bg-background p-6">
      <BodyText className="text-xs text-foreground-muted">
        ...
        <StrongText className="text-xs">Update the runbook</StrongText> needed a
        description of the code changes and considered reusing{" "}
        <StrongText className="text-xs">Review the diff</StrongText> -- but
        Review the diff already depends on Update the runbook, so reusing it
        would close a cycle back to itself. The DAG guard rejects the edge, and
        the agent falls back to spawning a fresh node,{" "}
        <StrongText className="text-xs">Summarize the diff</StrongText>,
        instead. Expand Update the runbook to see the rejection recorded.
      </BodyText>
