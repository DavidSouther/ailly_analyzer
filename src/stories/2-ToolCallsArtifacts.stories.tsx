...
const TRANSCRIPT: TranscriptItem[] = [
  ...
  },
  {
    ...
    id: "a1",
    text:
      "Sure -- let me look at the current loader first, then sketch a " +
      "streaming version.",
  },
  {
    kind: "tool-call",
...
function TodoUpdateMessage({
  ...
  return (
    <TurnRow role="Agent" avatar={<AssistantAvatar />}>
      <div className="max-w-md rounded border bg-background-hover p-2">
        <MutedText className="text-xs font-medium tracking-wide uppercase">
          {heading}
        </MutedText>
        <ul className="mt-1 flex flex-col gap-1">
          {items.map((entry) => (
            <li key={entry.label} className="flex items-center gap-1.5 text-sm">
              ...
              <span
                className={cn(
                  entry.done && "text-foreground-muted line-through",
                )}
              >
                {entry.label}
              </span>
    ...
    <div className="flex flex-col divide-y divide-border">
      {TRANSCRIPT.map((item) => {
        switch (item.kind) {
          ...
          case "assistant":
            return (
              <TurnRow key={item.id} role="Agent" avatar={<AssistantAvatar />}>
                <BodyText>{item.text}</BodyText>
              </TurnRow>
            );
          case "tool-call":
