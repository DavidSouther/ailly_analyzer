...
function ThinkingDisclosure({ text }: { text: string }) {
  ...
  return (
    <div className="pl-1">
      ...
      {expanded ? (
        <p className="mt-1 ml-5 max-w-md text-xs text-foreground-muted italic">
          {text}
        </p>
      ) : null}
    </div>
...
function SubagentInterface({ task }: { task: Task }) {
  if (task.toolCalls.length === 0) {
    return (
      <MutedText className="pl-1 text-xs">
        This subagent hasn&apos;t started yet.
      </MutedText>
    );
  }
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col gap-3 bg-background p-6">
      <BodyText className="text-xs text-foreground-muted">
        Refined orchestrator view -- the TODO list is the top-level surface.
        Expand a task to drill into its subagent&apos;s own interface; thinking
        stays collapsed as a supporting detail, and tool calls stay collapsed
        for quick review instead of scrolling past in flow.
      </BodyText>
