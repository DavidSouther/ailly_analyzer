...
- The line between the previous two bullets: a parameter a call recorded is in scope wherever that call appears, including an edit's replaced text or a write's content, because it is part of what the call did. A file's content on disk, or any reconstruction of one from a sequence of calls, is not — that is the artifact review this view sends the user elsewhere for. Parameters stay attached to the call that recorded them, collapsed by default and bounded when long; they never become a file-first surface.
